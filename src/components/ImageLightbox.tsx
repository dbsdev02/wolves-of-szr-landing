  ...
  return (
    <Dialog open={Boolean(item)} onOpenChange={(value) => !value && onClose()}>
      <DialogContent className="h-[92vh] max-w-[94vw] border-border bg-background/95 p-3 sm:p-5 [&>button]:hidden">
        ...
        {item && <>
          <Button variant="glass" size="icon" className="absolute left-5 top-1/2" onClick={showPrevious} aria-label="Previous image"><ChevronLeft /></Button>
          <Button variant="glass" size="icon" className="absolute right-5 top-1/2" onClick={showNext} aria-label="Next image"><ChevronRight /></Button>
